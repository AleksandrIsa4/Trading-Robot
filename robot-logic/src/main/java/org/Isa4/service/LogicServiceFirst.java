...
@Slf4j
@Service("logicServiceFirst")
@RequiredArgsConstructor
public class LogicServiceFirst implements LogicServiceImpl {
    ...
    // Создание параметров новой заявки
    ...
    // Получение из базы данных последних значений инструмента
        ...
        try {
            ...
            if (averageChena <= minPrice) {
                ...
                if (averageChena <= minPrice) {
                    ...
                    Rounding(priceBuy);
                    Rounding(priceSell);
                    //  priceBuy1 = new BigDecimal(informationTool.getBid() + Math.floor(((informationTool.getOffer() - informationTool.getBid()) / 2) / secPriceStep) * secPriceStep - Math.floor(takeProfit / 2) * secPriceStep);
    ...
    private float Rounding(float price) {
        ...
        //   int sec=BigDecimal.valueOf(secPriceStep).scale();
        //    BigDecimal bd = new BigDecimal(price);

        // округление до ближайшего числа с указанной точностью
        //   bd = bd.setScale(sec, RoundingMode.HALF_UP);
        //    return bd.floatValue();
        //   long price2=(long)Math.floor(price/secPriceStep);
        //     float price3=price2*secPriceStep;
        //    return price3;
    }
